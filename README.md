...
| **노트 상세** | 요약·원문 보기, 카테고리/프로젝트 지정, 즐겨찾기 토글, 삭제. 관리자 계정은 공개 여부(`is_public`) 설정 가능 |
...
│       ├── notes/[id]/route.ts   # 노트 PATCH/DELETE
...
│   └── theme-toggle.tsx
├── lib/                          # 서버/클라이언트 공용 로직
│   ├── public-notes.ts           # 공개 관람용 관리자 노트 조회 로직
│   ├── supabase/
...
├── supabase/
│   └── migrations/               # DB 마이그레이션 (001~015 순서 적용)
├── scripts/
...
| PATCH | `/api/notes/[id]` | 노트 수정 (`category_id`, `category_ids`, `is_favorite`, 관리자 전용 `is_public`) |
...
- **동작 요약** (`middleware.ts`)
  ...
  - `/public` 경로는 비회원 공개 관람을 위해 로그인 없이 접근 가능합니다.
...
- 관리자는 노트 상세에서 `is_public` 값을 켜고 끄며 공개 관람 노출 여부를 제어할 수 있습니다

---

## 6. 데이터베이스 구조 (Supabase / PostgreSQL)

DB 구조는 `supabase/migrations/` 안의 SQL 파일을 번호 순서(001 → 015)대로 적용한 상태를 기준으로 합니다.
...
| `015_notes_public_visibility.sql` | `notes.is_public` (BOOLEAN, 기본 true) 추가. 관리자 공개/숨김 제어 및 공개 관람 필터를 위한 인덱스 추가 |

### 테이블 역할 요약

- **notes**: 업로드한 파일의 메타정보·요약·상태. `user_id`, `category_id`, `project_id`, `is_favorite`, `is_public` 로 사용자/카테고리/프로젝트/즐겨찾기/공개여부와 연결
...
- **공개 관람 로직**: `lib/public-notes.ts` — 관리자 계정 노트 중 공개(`is_public`) 상태인 요약 데이터만 조회
...
App Router 기준으로 `app/`(페이지·API), `components/`, `lib/`(supabase, llm, utils, public-notes), `types/`, `supabase/migrations/` 로 역할이 나뉘어 있음.

### 코드 정리

서버에서 데이터·인증을 처리한 뒤 클라이언트에 초기값 전달. 상수·타입·인증은 `lib/utils`, `types` 에 모아 두고, Supabase 는 server / client / admin 세 가지 용도로 나누어 사용. 공개 관람은 인증 영역과 분리된 별도 라우트/조회 로직으로 관리.

...
export async function PATCH(
  ...
  try {
    ...
    if (categoryIds === undefined && legacyCategoryId === undefined && isFavorite === undefined && isPublic === undefined) {
      return NextResponse.json({ error: 'category_ids, category_id, is_favorite 또는 is_public 중 하나가 필요합니다.' }, { status: 400 })
    }
    ...
    if (isPublic !== undefined) {
      if (!isAdmin(user.email)) {
        return NextResponse.json({ error: '공개 설정은 관리자만 변경할 수 있습니다.' }, { status: 403 })
      }
      const { error: updateError } = await admin
        .from('notes')
        ...
        .eq('user_id', user.id)
      if (updateError) {
        return NextResponse.json({ error: updateError.message || '공개 설정 변경에 실패했습니다.' }, { status: 500 })
      }
    }

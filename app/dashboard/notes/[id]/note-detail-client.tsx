...
export default function NoteDetailClient({
  ...
  const isPublic = currentNote.is_public !== false
  ...
  const handlePublicToggle = async () => {
    const next = !isPublic
    setCurrentNote((n) => ({ ...n, is_public: next }))
    ...
    if (!res.ok) {
      setCurrentNote((n) => ({ ...n, is_public: !next }))
      const data = await res.json().catch(() => ({}))
      alert(data.error || '공개 설정 변경에 실패했습니다.')
    } else {
      router.refresh()
    }
  }
              ...
              {isAdmin && (
                <button
                  ...
                  className={`rounded-lg border px-3 py-1.5 text-sm font-medium disabled:opacity-50 ${
                    isPublic
                      ? 'border-[var(--success)]/40 bg-[var(--success-muted)] text-[var(--success)]'
                      : 'border-[var(--error)]/40 bg-[var(--error-muted)] text-[var(--error)]'
                  }`}
                  title={isPublic ? '공개 대상에게 보이는 상태입니다.' : '공개 대상에게 숨겨진 상태입니다.'}
                >
                  {savingPublic
                    ? '저장 중...'
                    : isPublic
